...
public class CA_History : MonoBehaviour {
    ...
    public int birth = 3;
    private float CubeAge;
    private float _layerdensity;
    private string layerDensityText = "Layer Density";
    public UnityEngine.UI.Text layerdensity;
    ...
    private void UpdateGrid()
    {
        ...
        for (int i = 0; i < columns; i++)
        {
            for (int j = 0; j < rows; j++)
            {
                ...
                if (currentCubeState == 1)
                {
                    ...
                    if (neighborsCount == starvation || neighborsCount == overPopulation)
                    {
                        ...
                        //added
                        ageOfCells[i, j]++;
                        grid[i, j].GetComponent<CA_Cube>().SetAge(ageOfCells[i, j]);
                    }
        ...
        //Added for Layer density
        if (alive != 0)
        {
            _layerdensity = (float)alive/ ((float)rows * (float)columns);
            Debug.Log(alive);
            layerDensityText += "\n" + "Layer No:" + recordedGridStates + ": " + _layerdensity.ToString();
            layerdensity.text = layerDensityText;
        }
    }
        ...
        for (int i = 0; i < columns; i++)
        {
            for (int j = 0; j < rows; j++)
            {
                ...
                if (gridCube.GetComponent<CA_Cube>().GetState() == 1)
                {
                    ...
                    //Destroy(newObj.GetComponent<CA_Cube>()); - edited
                    newObj.GetComponent<CA_Cube>().updateState = false; // added
                    ...
                    // Copy age information - edited
                    newObj.GetComponent<CA_Cube>().SetAge(ageOfCells[i, j]);// added
                }                        
            }
        }
    }
